# EcoMap ΓÇö Modular Unified Teacher-Student Pipeline
...
### Step 1 ΓÇö Download the datasets

The three reference datasets are available here:

> [Google Drive ΓÇö Dataset Folder](https://drive.google.com/drive/folders/1h2mY0to3B52E_IKbG4DPKqhckzK-08o-?usp=sharing)

Unzip the folders into your **project root**.

### Step 2 ΓÇö Choose or edit a config file
...
| `barcode_metadata.csv` | Metadata ΓÇö must include `x_coord`, `y_coord`, `patient_id` |
| `label_mapping.json` | Class name ΓåÆ index mapping |
...
| Property | Detail |
|----------|--------|
| Input dimensionality | 1177D ΓÇö UNI (1024D) + scVI (128D) + Cell Composition (25D for GEO, 15D for ZENODO) |
...
| Property | Detail |
|----------|--------|
| Input dimensionality | 1024D ΓÇö UNI embeddings only, PCA-reduced relative to teacher |
| Hidden layers | [128, 64, 32] |
| Output | 5-class ecotype predictions |

> The student intentionally uses fewer inputs. Its lower accuracy (~79% vs teacher ~88%) reflects the information cost of dropping gene expression and cell composition data ΓÇö this is expected, not a bug.
...
embeddings:
  image_encoder:
    pca_variance: 0.6   # Reduces 1024D ΓåÆ ~150D
```
...
Γöé
Γö£ΓöÇΓöÇ TEACHER_UNIFIED_[DATASET]_0.6_PCA_ver5/
Γöé   Γö£ΓöÇΓöÇ training/metrics/       # Cross-validation metrics, per-fold results
Γöé   Γö£ΓöÇΓöÇ training/models/        # Fold-specific teacher model weights
Γöé   ΓööΓöÇΓöÇ .working/               # Preprocessed embeddings, PCA models
Γöé
ΓööΓöÇΓöÇ STUDENT_UNIFIED_[DATASET]_0.6_PCA_ver5/
    Γö£ΓöÇΓöÇ training/metrics/       # Student accuracy, F1, predictions CSV
    Γö£ΓöÇΓöÇ training/models/        # Final student model weights
    Γö£ΓöÇΓöÇ post-training/visualizations/
    Γöé   Γö£ΓöÇΓöÇ spatial/            # Ecotype maps with tissue coordinates
    Γöé   Γö£ΓöÇΓöÇ spatial_confidence/ # Prediction confidence heatmaps
    Γöé   Γö£ΓöÇΓöÇ spatial_comparison/ # Teacher vs student side-by-side
    Γöé   Γö£ΓöÇΓöÇ neighborhood/       # Neighbor composition analysis
    Γöé   ΓööΓöÇΓöÇ preprocessing/      # Embedding quality metrics
    ΓööΓöÇΓöÇ pipeline_execution.log  # Full execution log
```
...
Spatial visualizations require `x_coord` and `y_coord` in `barcode_metadata.csv`. If those columns are missing, only the spatial outputs are skipped ΓÇö all other visualizations will still generate.
